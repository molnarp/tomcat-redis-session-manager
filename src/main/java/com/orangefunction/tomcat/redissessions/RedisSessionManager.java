...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
    ...
    public Session createSession(String requestedSessionId) {
        ...
        try {
            ...
            if (null != requestedSessionId) {
                ...
                if (jedis.setnx((redisKeyPrefix + sessionId).getBytes(), NULL_SESSION) == 0L) {
                    sessionId = null;
                }
            } else {
                do {
                ...
                } while (jedis.setnx((redisKeyPrefix + sessionId).getBytes(), NULL_SESSION) == 0L); // 1 = key set; 0 = key already existed
            }
    ...
    public String[] keys() throws IOException {
        ...
        try {
            jedis = acquireConnection();
            Set<String> keySet = jedis.keys(redisKeyPrefix + "*");
            error = false;
    ...
    public byte[] loadSessionDataFromRedis(String id) throws IOException {
        ...
        try {
            ...
            jedis = acquireConnection();
            byte[] data = jedis.get((redisKeyPrefix + id).getBytes());
            error = false;
    ...
    protected boolean saveInternal(Jedis jedis, Session session, boolean forceSave) throws IOException {
        ...
        try {
            ...
            byte[] binaryId = (redisKeyPrefix + redisSession.getId()).getBytes();
